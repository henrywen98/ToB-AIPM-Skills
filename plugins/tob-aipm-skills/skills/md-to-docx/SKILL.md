...
name: md-to-docx
description: >-
  把 Markdown 文件转成带格式的 Word 文档（.docx）。用 pandoc + reference-doc 模板 + python-docx 后处理。
  当用户说"转 Word"、"生成 docx"、"md 转 word"、"markdown 转文档"、"/md-to-docx"时触发。
  在 ToB 工作流里给 phase-confirm-doc 出客户用印 docx、给 feature-spec 出独立 SPEC 快照（不签字、举证用）。
  Not for: 从零写 Markdown 内容、转其他格式。
---
...
```bash
ls "<md文件路径>"
ls "${CLAUDE_PLUGIN_ROOT}/templates/reference.docx"
```
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/build_template.py <源docx文件>
```

这会基于源文档的样式结构重新生成 `${CLAUDE_PLUGIN_ROOT}/templates/reference.docx`。之后重新转换文档即可生效。
